...
namespace structures
{
	...
	void Heap<T>::push(const int priority, const T& data)
	{
		...
		int indexCurrent = this->list_->size() - 1;
		int indexParent = getParentIndex(indexCurrent);

		while (
			indexParent != -1 &&
			this->list_->operator[](indexCurrent)->getPriority()
			<
			this->list_->operator[](indexParent)->getPriority()
		)
		{
			DSRoutines::swap(
				this->list_->operator[](indexCurrent),
				this->list_->operator[](indexParent)
			);
	...
	T Heap<T>::pop()
	{
		...
		if (this->list_->size() == 1)
		{
			...
			item = this->list_->removeAt(indexOfPeek());
		}
		else
		{
			// Grab highest priority item
			item = this->list_->operator[](indexOfPeek());

			// Put last item in place of removed item
			this->list_->operator[](0) = this->list_->removeAt(this->list_->size() - 1);
			...
			while (
				...
				this->list_->operator[](indexCurrent)->getPriority()
				>
				this->list_->operator[](indexSon)->getPriority()
			)
			{
				DSRoutines::swap(
					this->list_->operator[](indexCurrent),
					this->list_->operator[](indexSon)
				);
	...
	inline int Heap<T>::getGreaterSonIndex(const int index)
	{
		...
		sonLeft = indexLeft < this->list_->size() ? 
			this->list_->operator[](indexLeft) : nullptr;
		sonRight = indexRight < this->list_->size() ? 
			this->list_->operator[](indexRight) : nullptr;
