...
namespace structures
{
	...
	inline void PriorityQueueSortedArrayList<T>::push(const int priority, const T& data)
	{
		...
		{
			// New item has priority of at least peek
			this->list_->add(item);
		}
		else
		{
			// Find item in queue with priority of at least new item
			int i = 0;
			while (priority < this->list_->operator[](i)->getPriority())
			{
				i++;
			}

			// Place new item behind the one found
